...
# Buka file CSV untuk menulis hasil deteksi
csv_file = open('revprogramdeteksi.csv', 'w', newline='')
csv_writer = csv.writer(csv_file)
csv_writer.writerow(['Timestamp', 'R', 'G', 'B', 'Detected Color'])
...
while True:
    ...
    # Ambil nilai RGB
    r, g, b = pixel_center[::-1]
    
    # Buat timestamp
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    
    # Tulis ke file CSV
    csv_writer.writerow([timestamp, r, g, b, color_pred])  # Menggunakan color_pred di sini
    
    # Cetak ke terminal
    print(f"{timestamp} - RGB: ({r}, {g}, {b}) - Detected Color: {color_pred}")  # Menggunakan color_pred di sini
...
cap.release()
csv_file.close()  # Pastikan file CSV ditutup
